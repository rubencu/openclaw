...
const GENERIC_PROGRESS_TITLES = new Set(["command", "reasoning", "tool"]);
...
function resolvePreviewToolProgressLine(payload: {
  progressText?: string;
  summary?: string;
  title?: string;
  name?: string;
}): string | undefined {
  const candidates = [payload.progressText, payload.summary, payload.title, payload.name];
  for (const candidate of candidates) {
    const normalized = candidate?.replace(/\s+/g, " ").trim();
    if (!normalized) {
      continue;
    }
    if (GENERIC_PROGRESS_TITLES.has(normalized.toLowerCase())) {
      continue;
    }
    return normalized;
  }
...
export const dispatchTelegramMessage = async ({
  ...
  const pushPreviewToolProgress = (line?: string) => {
    ...
    answerLane.hasProgressPreview = true;
  ...
  const resetDraftLaneState = (lane: DraftLaneState) => {
    ...
    lane.hasProgressPreview = false;
  };
  const rotateAnswerLaneForNewAssistantMessage = async () => {
    if (!answerLane.hasStreamedMessage) {
      if (activePreviewLifecycleByLane.answer !== "transient") {
  ...
  try {
    ...
    const deliverLaneText = createLaneTextDeliverer({
      ...
      draftMaxChars,
      finalizeAnswerMessagePreviewsInPlace: false,
      applyTextToPayload,
      sendPayload,
      flushDraftLane,
      ...
      ({ queuedFinal } = await telegramDeps.dispatchReplyWithBufferedBlockDispatcher({
        ...
        replyOptions: {
          ...
          onItemEvent: async (payload) => {
            pushPreviewToolProgress(resolvePreviewToolProgressLine(payload));
          },
